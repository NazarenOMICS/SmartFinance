...
export default function Accounts({ settings, refreshSettings, onAccountDeleted }) {
  ...
  async function load(options = {}) {
    ...
    try {
      const [accounts, consolidated, links] = await Promise.all([
        api.getAccounts(),
        api.getConsolidatedAccounts(),
        api.getAccountLinks().catch(() => []),
      ]);
    ...
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-2">
        <MetricCard
          ...
          value={fmtMoney(state.consolidated?.total ?? 0, state.consolidated?.currency ?? displayCurrency)}
