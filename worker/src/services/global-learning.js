...
async function fingerprintUser(userId) {
  ...
  const digest = await crypto.subtle.digest(
    "SHA-256",
    encoder.encode(`${GLOBAL_USER_PEPPER}:${String(userId || "")}`)
  );
...
async function getCategorySlug(db, userId, categoryId) {
  if (!categoryId) return null;
  const row = await db.prepare(
    "SELECT slug FROM categories WHERE user_id = ? AND id = ?"
  ).get(userId, Number(categoryId));
  const slug = String(row?.slug || "").trim();
  return slug || null;
}

async function maybePromoteGlobalAlias(db, normalizedPattern, categorySlug) {
...
export async function recordGlobalPatternLearning(db, userId, descBanco, categoryId, decision = "confirm") {
  const categorySlug = await getCategorySlug(db, userId, categoryId);
  ...
  const userFingerprint = await fingerprintUser(userId);
  ...
  if (safeDecision === "confirm") {
    await maybePromoteGlobalAlias(db, normalizedPattern, categorySlug);
  }

  return { normalizedPattern, categorySlug, decision: safeDecision };
}

export async function findGlobalAliasMatch(db, descBanco) {
