...
function removeBankArtifacts(value) {
...
export function normalizeBankDescription(input, _context = {}) {
...
function scoreMerchantPhrase(phrase) {
...
function bestSignificantPhrase(tokens) {
    const candidates = [
        ...ngrams(tokens, 3),
        ...ngrams(tokens, 2),
        ...ngrams(tokens, 1),
...
function isChannelOnlyDescription(normalized) {
...
function strippedChannelNoise(strippedNoise) {
...
export function extractMerchant(normalizedDescription, dictionary = []) {
    const normalizedResult = normalizeBankDescription(normalizedDescription);
    const normalized = normalizedResult.normalized;
    if (normalized.length === 0)
        return { merchant_key: null, display_guess: null, confidence: 0, method: "empty", stripped_noise: normalizedResult.stripped_noise, skipped_reason: "empty_description" };
    ...
    if (isChannelOnlyDescription(normalized) || strippedChannelNoise(normalizedResult.stripped_noise))
        return { merchant_key: null, display_guess: null, confidence: 0, method: "noise_only", stripped_noise: normalizedResult.stripped_noise, skipped_reason: "channel_or_transfer" };
    ...
    if (token && token.length >= 3)
        return { merchant_key: token, display_guess: titleCase(token), confidence: 0.68, method: "token", stripped_noise: normalizedResult.stripped_noise, skipped_reason: null };
    return { merchant_key: null, display_guess: null, confidence: 0, method: "noise_only", stripped_noise: normalizedResult.stripped_noise, skipped_reason: "noise_only" };
}
export function deriveRuleIdentity(descBanco, context = {}, dictionary = []) {
    ...
    if (merchant.merchant_key === null || isGenericRulePattern(merchant.merchant_key)) {
        return { skipped: true, skippedReason: merchant.skipped_reason || "generic_or_empty_merchant", normalized_pattern: normalized, merchant_key: null, merchant_scope: "", account_scope: context.accountId || "", currency_scope: context.currency || "", direction: context.direction || "any", confidence: merchant.confidence, merchant };
    }
    return { skipped: false, skippedReason: null, normalized_pattern: merchant.merchant_key, merchant_key: merchant.merchant_key, merchant_scope: merchant.merchant_key, account_scope: context.accountId || "", currency_scope: context.currency || "", direction: context.direction || "any", confidence: merchant.confidence, merchant };
}
export function getRuleScopes(rule) {
    ...
    return { merchant_scope: normalizeRulePattern(String(rule.merchant_scope || merchantKey || normalizedPattern)), account_scope: String(rule.account_id || ""), currency_scope: String(rule.currency || ""), direction: (rule.direction || "any") };
}
...
function recencyFactor(value) {
    ...
    if (days > 90)
        return Math.max(0.72, 1 - (days - 90) / 3650);
    return 1;
}
function scopeBonus(rule, tx) {
    ...
    if (direction !== "any" && direction === directionForAmount(tx.monto))
        bonus += 0.05;
    return bonus;
}
export function scoreRuleMatch(tx, rule, dictionary = []) {
    if ((rule.mode || "suggest") === "disabled")
    ...
    const direction = rule.direction || "any";
    if (rule.account_id && rule.account_id !== (tx.account_id || null))
        return Number.NEGATIVE_INFINITY;
    if (rule.currency && rule.currency !== (tx.moneda || null))
        return Number.NEGATIVE_INFINITY;
    if (direction !== "any" && direction !== directionForAmount(tx.monto))
        return Number.NEGATIVE_INFINITY;
    let base = 0;
...
function ruleSourcePriority(rule) {
    ...
    if (source === "manual" || source === "learned" || source === "guided")
        return 3;
    if (source === "seed")
        return 1;
    return 2;
}
function ruleScopeSpecificity(rule) {
    return (rule.account_id ? 1 : 0) + (rule.currency ? 1 : 0) + ((rule.direction || "any") !== "any" ? 1 : 0);
}
function ruleMerchantKey(rule) {
    return normalizeRulePattern(String(rule.merchant_key || rule.merchant_scope || ""));
}
function shouldPreferRule(candidate, current) {
    ...
    if (fallbackSourceDelta !== 0)
        return fallbackSourceDelta > 0;
    return ruleScopeSpecificity(candidate.rule) > ruleScopeSpecificity(current.rule);
}
export function matchRules(tx, rules = [], dictionary = []) {
    let best = null;
...
export function classifyTransaction(tx, rules = [], ruleRejections = [], settings = {}, dictionary = []) {
...
export function explainCategorization(decision) {
...
export function buildManualRuleUpsert(tx, categoryId, scopePreference = tx.account_id ? "account" : "global", dictionary = []) {
