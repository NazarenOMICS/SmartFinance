...
SELECT
  ...
  keep.user_id,
  keep.pattern,
  keep.normalized_pattern,
  COALESCE(NULLIF(keep.merchant_key, ''), NULLIF(keep.normalized_pattern, ''), keep.pattern),
  COALESCE(NULLIF(keep.merchant_key, ''), NULLIF(keep.normalized_pattern, ''), keep.pattern),
  keep.account_id,
...
WHERE keep.id IN (
  ...
  GROUP BY
    candidate.user_id,
    COALESCE(NULLIF(candidate.merchant_key, ''), NULLIF(candidate.normalized_pattern, ''), candidate.pattern),
    COALESCE(candidate.account_id, ''),
    COALESCE(candidate.currency, ''),
    COALESCE(NULLIF(candidate.direction, ''), 'any')
);

UPDATE rules_canonical
SET match_count = (
      SELECT MAX(source_rules.match_count)
      FROM rules source_rules
      WHERE source_rules.user_id = rules_canonical.user_id
        AND COALESCE(NULLIF(source_rules.merchant_key, ''), NULLIF(source_rules.normalized_pattern, ''), source_rules.pattern) = rules_canonical.merchant_scope
        AND COALESCE(source_rules.account_id, '') = rules_canonical.account_scope
        AND COALESCE(source_rules.currency, '') = rules_canonical.currency_scope
        AND COALESCE(NULLIF(source_rules.direction, ''), 'any') = rules_canonical.direction
    ),
    confidence = (
      SELECT MAX(source_rules.confidence)
      FROM rules source_rules
      WHERE source_rules.user_id = rules_canonical.user_id
        AND COALESCE(NULLIF(source_rules.merchant_key, ''), NULLIF(source_rules.normalized_pattern, ''), source_rules.pattern) = rules_canonical.merchant_scope
        AND COALESCE(source_rules.account_id, '') = rules_canonical.account_scope
...
CREATE INDEX IF NOT EXISTS idx_rule_match_log_rule
  ON rule_match_log (user_id, rule_id, created_at DESC);
